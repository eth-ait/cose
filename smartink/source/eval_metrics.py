...
from smartink.loss.chamfer import chamfer_distance_np_var_len_normalized
...
class MetricEngine(object):
  ...
  def chamfer(cls, targets, predictions):
    return [chamfer_distance_np_var_len_normalized((gt, pred)) for gt, pred in zip(targets, predictions)]
  ...
  def chamfer_distance(self, targets, predictions, return_all=True):
    ...
    if self.ignore_pen and self.ignore_pen_step:
      cd = [chamfer_distance_np_var_len_normalized((gt[:-1, 0:2], pred[:-1, 0:2])) for gt, pred in zip(targets, predictions)]
    elif self.ignore_pen:
      cd = [chamfer_distance_np_var_len_normalized((gt[:, 0:2], pred[:, 0:2])) for gt, pred in zip(targets, predictions)]
    elif self.ignore_pen_step:
      cd = [chamfer_distance_np_var_len_normalized((gt[:-1], pred[:-1])) for gt, pred in zip(targets, predictions)]
    else:
      cd = [chamfer_distance_np_var_len_normalized((gt, pred)) for gt, pred in zip(targets, predictions)]
