...
class PredictiveInkModel(BaseModel):
  ...
  def __init__(self,
    ...
    self.embedding_normalization = None
    if layer_norm_embeddings:
      self.embedding_normalization = tf.keras.layers.LayerNormalization()
  ...
  def call(self, inputs, output_len=None, training=None, **kwargs):
    ...
    # as input.
      ...
      max_n_stroke = tf.reduce_min(input_tensor=input_num_strokes)
      
      # def get_random_inp_targets(n_samples, n_all_strokes):
      #   n_inputs_ = tf.random_uniform([1], minval=2, maxval=max_n_stroke, dtype=tf.int32)[0]
      #   i = tf.random_uniform([n_strokes], minval=0, maxval=1, dtype=tf.float32)
      #   target_idx_ = tf.round(i*tf.cast(input_num_strokes - 1, tf.float32))
      #   target_idx_ = tf.cast(target_idx_, tf.int32)
      # 
      #   input_range = tf.tile(tf.range(seq_len)[tf.newaxis, :], [n_strokes, 1])
      #   mask_ = tf.not_equal(input_range, tf.tile(target_idx_[:, tf.newaxis], [1, seq_len]))
      #   all_input_idx = tf.reshape(tf.where(mask_), [n_strokes, seq_len - 1, 2])
      #   shuffled = tf.random.shuffle(tf.transpose(all_input_idx[:, :max_n_stroke-1], [1,0,2]))
      #   gather_input_idx_ = tf.cast(tf.transpose(shuffled[:n_inputs_], [1,0,2]), tf.int32)
      #   return gather_input_idx_
      
      # Set 1
      n_inputs_1 = tf.random.uniform([1], minval=2, maxval=max_n_stroke, dtype=tf.int32)[0]
      i = tf.random.uniform([n_strokes], minval=0, maxval=1, dtype=tf.float32)
      target_idx_1 = tf.round(i*tf.cast(input_num_strokes - 1, tf.float32))
      target_idx_1 = tf.cast(target_idx_1, tf.int32)

      input_range = tf.tile(tf.range(seq_len)[tf.newaxis, :], [n_strokes, 1])
      mask_ = tf.not_equal(input_range, tf.tile(target_idx_1[:, tf.newaxis], [1, seq_len]))
      all_input_idx = tf.reshape(tf.compat.v1.where(mask_), [n_strokes, seq_len - 1, 2])
      shuffled = tf.random.shuffle(tf.transpose(a=all_input_idx[:, :max_n_stroke-1], perm=[1,0,2]))
      gather_input_idx_1 = tf.cast(tf.transpose(a=shuffled[:n_inputs_1], perm=[1,0,2]), tf.int32)
      
      # Set 2
      n_inputs_2 = tf.random.uniform([1], minval=2, maxval=max_n_stroke, dtype=tf.int32)[0]
      i = tf.random.uniform([n_strokes], minval=0, maxval=1, dtype=tf.float32)
      target_idx_2 = tf.round(i*tf.cast(input_num_strokes - 1, tf.float32))
      target_idx_2 = tf.cast(target_idx_2, tf.int32)

      input_range = tf.tile(tf.range(seq_len)[tf.newaxis, :], [n_strokes, 1])
      mask_ = tf.not_equal(input_range, tf.tile(target_idx_2[:, tf.newaxis], [1, seq_len]))
      all_input_idx = tf.reshape(tf.compat.v1.where(mask_), [n_strokes, seq_len - 1, 2])
      shuffled = tf.random.shuffle(tf.transpose(a=all_input_idx[:, :max_n_stroke-1], perm=[1,0,2]))
      gather_input_idx_2 = tf.cast(tf.transpose(a=shuffled[:n_inputs_2], perm=[1,0,2]), tf.int32)
      
      # Set 3
      n_inputs_3 = tf.random.uniform([1], minval=2, maxval=max_n_stroke, dtype=tf.int32)[0]
      i = tf.random.uniform([n_strokes], minval=0, maxval=1, dtype=tf.float32)
      target_idx_3 = tf.round(i*tf.cast(input_num_strokes - 1, tf.float32))
      target_idx_3 = tf.cast(target_idx_3, tf.int32)

      input_range = tf.tile(tf.range(seq_len)[tf.newaxis, :], [n_strokes, 1])
      mask_ = tf.not_equal(input_range, tf.tile(target_idx_3[:, tf.newaxis], [1, seq_len]))
      all_input_idx = tf.reshape(tf.compat.v1.where(mask_), [n_strokes, seq_len - 1, 2])
      shuffled = tf.random.shuffle(tf.transpose(a=all_input_idx[:, :max_n_stroke-1], perm=[1,0,2]))
      gather_input_idx_3 = tf.cast(tf.transpose(a=shuffled[:n_inputs_3], perm=[1,0,2]), tf.int32)
      
      # Set 4
      n_inputs_4 = tf.random.uniform([1], minval=2, maxval=max_n_stroke, dtype=tf.int32)[0]
      i = tf.random.uniform([n_strokes], minval=0, maxval=1, dtype=tf.float32)
      target_idx_4 = tf.round(i*tf.cast(input_num_strokes - 1, tf.float32))
      target_idx_4 = tf.cast(target_idx_4, tf.int32)

      input_range = tf.tile(tf.range(seq_len)[tf.newaxis, :], [n_strokes, 1])
      mask_ = tf.not_equal(input_range, tf.tile(target_idx_4[:, tf.newaxis], [1, seq_len]))
      all_input_idx = tf.reshape(tf.compat.v1.where(mask_), [n_strokes, seq_len - 1, 2])
      shuffled = tf.random.shuffle(tf.transpose(a=all_input_idx[:, :max_n_stroke-1], perm=[1,0,2]))
      gather_input_idx_4 = tf.cast(tf.transpose(a=shuffled[:n_inputs_4], perm=[1,0,2]), tf.int32)
      
      max_len = tf.reduce_max(input_tensor=[n_inputs_1, n_inputs_2, n_inputs_3, n_inputs_4])

      gather_input_idx_1 = tf.pad(tensor=gather_input_idx_1, paddings=[[0, 0], [0, max_len-n_inputs_1], [0,0]])
      gather_input_idx_2 = tf.pad(tensor=gather_input_idx_2, paddings=[[0, 0], [0, max_len-n_inputs_2], [0,0]])
      gather_input_idx_3 = tf.pad(tensor=gather_input_idx_3, paddings=[[0, 0], [0, max_len-n_inputs_3], [0,0]])
      gather_input_idx_4 = tf.pad(tensor=gather_input_idx_4, paddings=[[0, 0], [0, max_len-n_inputs_4], [0,0]])

      gather_input_idx = tf.concat([gather_input_idx_1, gather_input_idx_2, gather_input_idx_3, gather_input_idx_4], axis=0)
      pred_input_seq_len = tf.concat([tf.ones([n_strokes], dtype=tf.int32)*n_inputs_1, tf.ones([n_strokes], dtype=tf.int32)*n_inputs_2, tf.ones([n_strokes], dtype=tf.int32)*n_inputs_3, tf.ones([n_strokes], dtype=tf.int32)*n_inputs_4], axis=0)

      gather_target_idx = tf.stack([
          tf.tile(batch_idx, [4]),
          tf.concat([target_idx_1, target_idx_2, target_idx_3, target_idx_4], axis=0)
          ], axis=-1)
    ...
    pos = tf.reshape(inputs["start_coord"], [n_strokes, seq_len, 2])
    context_pos = tf.gather_nd(pos, gather_input_idx)
    target_pos = tf.expand_dims(tf.gather_nd(pos, gather_target_idx), axis=1)
    predicted_embedding = self.predictive_model(inputs=pred_input, input_cond=context_pos, target_cond=target_pos, seq_len=pred_input_seq_len)
    ...
    if self.position_model is not None:
      predicted_pos = self.position_model(inputs=pred_input, input_cond=context_pos, target_cond=None, seq_len=pred_input_seq_len)
  ...
  def predict_embedding(self, embeddings, target_idx, seq_lens, input_idx=None, input_type="leave_one_out", start_positions=None):
    ...
    pos = tf.reshape(start_positions, [n_strokes, seq_len, 2])
    context_pos = tf.gather_nd(pos, gather_input_idx)
    target_pos = tf.expand_dims(tf.gather_nd(pos, gather_target_idx), axis=1)
    out_ = self.predictive_model(inputs=pred_input,
  ...
  def predict_embedding_ar(self, inp_embeddings, inp_pos=None, target_pos=None, seq_len=None, greedy=True):
    ...
    out_ = self.predictive_model(inputs=inp_embeddings,
                                 input_cond=inp_pos,
                                 target_cond=target_pos,
                                 seq_len=seq_len)
  ...
  def predict_position_ar(self, inp_embeddings, inp_pos=None, target_pos=None, seq_len=None):
    # Determine ink model inputs and targets.
    if seq_len is None:
      n_strokes = tf.shape(input=inp_embeddings)[1]
      seq_len = tf.ones_like(inp_embeddings[:, 0, 0], dtype=tf.int32)*n_strokes
    out_ = self.position_model(inputs=inp_embeddings,
                               input_cond=inp_pos,
                               target_cond=target_pos,
                               seq_len=seq_len)
    out_["position_sample"] = self.position_model.output_layer.draw_sample(out_, greedy=False)
    return out_
  ...
  def loss(self, predictions, targets, seq_len=None, prefix="", training=False):
    ...
    if self.run_mode == C.RUN_ESTIMATOR:
      loss_metric_dict["loss"] = tf.compat.v1.metrics.mean(loss_val_dict["loss"])
